...
class PayPal {
	...
	// private $_log_file = null; // path to the log file for ipn response
	...
	public function __construct($cmd = '_cart'){
		...
		$this->_cmd = $cmd;
		$this->return_url = WEBSITE_URL."/?page=payment_successful";
		$this->cancel_url = WEBSITE_URL."/?page=payment_cancelled";
	...
	private function standardFields(){
		...
		switch($this->_cmd){
			...
			break;
			case '_xclick':
			// not using for this website but can be used in future improvements
			break;
		}
		
	}
	
	// populate all details of the user 
	...
	private function validIpn(){
		$hostname = gethostbyaddr($_SERVER['REMOTE_ADDR']); //host by address
