	...
	<div id="cart">
		...
		<?php foreach($array as $product){ ?>
				<tr>
					...
					<td><?php $colour = $newColour->getColour($product['colour']);
						echo $colour['colour']; ?>
					</td>
				<?php 
					if(!empty($product['size_number'])){
						$size = $newSize->getSizeNumber($product['size_number']);
						echo "<td>".$size['size_number']."</td>";
					} else {
						$size = $newSize->getSizeLetter($product['size_letter']);
						echo "<td>".$size['size_letter']."</td>";
					}
				?>
		...
		<?php } ?>
			...
			<div><a href="?page=payment_with_cash" class="cash">Pay by Cash</a></div> 
		</form>
	</div>	
	<div class="dn">
